...
* **Версия сервиса:** 0.1.4 (MVP in active dev)
...
1. **Главный экран / Хаб (`pages/LandingPage.vue`):**
   * **Состояние гостя:** промо-страница, партнерские блоки вузов (ГУУ, СПбГУ, СГТУ, «Твой Ход»), кнопки перехода на регистрацию/авторизацию.
   * **Состояние игрока:** карточка профиля (ник, место в рейтинге, капитал, энергия) + кнопка «Продолжить игру» (переход на игровое поле).

2. **Страница аутентификации и онбординга (`pages/AuthPage.vue`):**
   * Полноэкранный каркас мастера `AuthWizardLayout.vue` с навигационной шапкой (`[← H]`, капсула «Узнаем немного», `[→]`).
   * **Единый каркас шагов 1–4:** ширина и высота белой карточки фиксированы между этапами и масштабируются с вьюпортом (`rem` / `vh` / `vw` / `clamp`, ландшафт).
   * **Модульный мастер регистрации (`AuthWizard.vue`):**
     * **Шаг 1 («Познакомимся» / `RegisterStepPersonal.vue`):** Сплит `form`. ФИО, вуз, направление, курс + 3D-иллюстрация `step-personal.png`.
     * **Шаг 2 («Защитимся» / `RegisterStepSecurity.vue`):** Сплит `form`. Email, пароль, подтверждение + 3D-иллюстрация `step-security.png`.
   ...
   * **Экран входа (Логин):** ещё не реализован (`/auth?mode=login` ведёт на ту же страницу мастера). `AuthModal.vue` — устаревший черновик, в маршрутах не используется.
...
└── src/
    ...
    │   │   │   ├── AuthModal.vue             # не используется в текущем флоу
    │   │   │   ├── AuthWizard.vue
...
4. **Главный экран:**
   * Гость / игрок, партнёры вузов, переход в `/game`.
...
- [ ] Реализовать форму входа для уже зарегистрированных игроков (логин по email/паролю).
...
2. **Графика и дизайн-система (Zero-Emoji Policy):**
   ...
   * Иконки — только **SVG** (inline или компоненты).
   * Капсульные инпуты и кнопки: `border-radius: 9999px`.
   * Без hover-подъёма (`translateY`) и без появления обводки у кнопок.
   * 3D-постеры онбординга лежат в `src/assets/images/auth/` с фиксированными именами (`step-personal.png`, `step-security.png`, `hobby-*.png`).
